...
import {
  ...
  Activity, KeyRound, ChevronRight,
} from 'lucide-react'

const GOALS = ['Weight Loss', 'Muscle Gain', 'Maintain Weight', 'General Health']
const PREFERENCES = ['Vegetarian', 'Vegan', 'Non-Vegetarian', 'Eggetarian']
const BODY_TYPES = ['Ectomorph', 'Mesomorph', 'Endomorph']
const GENDERS = ['Male', 'Female', 'Other']
const CONDITIONS = ['None', 'Diabetic', 'Hypertension', 'PCOS', 'Thyroid', 'Heart Condition', 'Other']
const ALLERGY_OPTIONS = ['Nuts', 'Dairy', 'Gluten', 'Eggs', 'Soy', 'Shellfish', 'Fish', 'Wheat']
...
export default function UserProfile() {
  ...
  const toggleEditAllergy = (allergy: string) => {
    ...
    updateEdit('allergies', current.includes(allergy)
  ...
  const toggleEditCondition = (condition: string) => {
    ...
    updateEdit('conditions', current.includes(condition)
      ...
      <PageWrapper title="Patient Profile">
        <div style={{ textAlign: 'center', padding: '80px' }}>
          ...
          <button
            ...
            style={{ marginTop: '20px', padding: '12px 24px', borderRadius: '14px', background: '#1a73e8', border: 'none', color: 'white', fontWeight: '700', cursor: 'pointer' }}
          >
            Back to Dashboard
          </button>
        </div>
  ...
  return (
    <PageWrapper title="Patient Profile">
      <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <button
          onClick={() => navigate('/dashboard')}
          style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'none', border: 'none', color: '#8a9bc4', fontWeight: '600', fontSize: '13px', cursor: 'pointer', width: 'fit-content', padding: '4px 0' }}
        >
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '28px', border: '1px solid #e8eef8', boxShadow: '0 2px 12px rgba(26,115,232,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <div>
              <div style={{ fontSize: '22px', fontWeight: '800', color: '#0d1b3e', letterSpacing: '-0.5px', marginBottom: '6px' }}>
                {user.name}
              </div>
          ...
          <div style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
            {!isEditing ? (
              <>
                <button
                  onClick={() => navigate(`/users/${id}/plan`)}
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 18px', borderRadius: '12px', background: '#eef3ff', border: '1px solid #dbe8ff', color: '#1a73e8', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}
                >
                  <UtensilsCrossed size={15} /> Diet Plan
                </button>
                <button
                  onClick={handleEdit}
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 18px', borderRadius: '12px', background: 'linear-gradient(135deg, #1a73e8, #1557b0)', border: 'none', color: 'white', fontSize: '13px', fontWeight: '700', cursor: 'pointer', boxShadow: '0 3px 12px rgba(26,115,232,0.3)' }}
                >
                  <Edit2 size={15} /> Edit
                </button>
              </>
              ...
              <>
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 18px', borderRadius: '12px', background: '#f8fafd', border: '2px solid #e8eef8', color: '#4a5568', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}
                >
                  <X size={15} /> Cancel
                </button>
                <button
        ...
        {activeTab === 'profile' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <input
                      ...
                      style={inputStyle}
                      value={editForm.age ?? ''}
                      onChange={(e) => updateEdit('age', e.target.value)}
                      onFocus={(e) => { e.target.style.border = '2px solid #1a73e8'; e.target.style.background = '#fafcff' }}
                      onBlur={(e) => { e.target.style.border = '2px solid #e8eef8'; e.target.style.background = '#f8fafd' }}
                    />
                  ) : (
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <div style={{ ...valueStyle, display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <Phone size={14} color="#8a9bc4" /> {user.phone || '—'}
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' as const }}>
                      {GENDERS.map((g) => (
                        <button key={g} type="button" onClick={() => updateEdit('gender', g)} style={chipStyle((editForm.gender ?? user.gender) === g)}>{g}</button>
                      ))}
                    </div>
                  ) : (
            ...
            <div style={cardStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
                <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: '#f0fdfa', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' }}>
                  ⚖️
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '20px' }}>
                <div>
                  ...
                  {isEditing ? (
                    <input type="number" style={inputStyle} value={editForm.weight ?? ''}
                      onChange={(e) => updateEdit('weight', e.target.value)}
                      onFocus={(e) => { e.target.style.border = '2px solid #1a73e8'; e.target.style.background = '#fafcff' }}
                      onBlur={(e) => { e.target.style.border = '2px solid #e8eef8'; e.target.style.background = '#f8fafd' }}
                    />
                  ) : (
                ...
                <div>
                  ...
                  {isEditing ? (
                    <input type="number" style={inputStyle} value={editForm.height ?? ''}
                      onChange={(e) => updateEdit('height', e.target.value)}
                      onFocus={(e) => { e.target.style.border = '2px solid #1a73e8'; e.target.style.background = '#fafcff' }}
                      onBlur={(e) => { e.target.style.border = '2px solid #e8eef8'; e.target.style.background = '#f8fafd' }}
                    />
                  ) : (
                ...
                <div>
                  ...
                  <span style={{
                    padding: '5px 12px', borderRadius: '40px', fontSize: '13px', fontWeight: '700',
                    background: user.bmiCategory === 'Normal' ? '#f0fdf4' : user.bmiCategory === 'Underweight' ? '#fffbeb' : user.bmiCategory === 'Overweight' ? '#fff7ed' : '#fff5f5',
                    color: user.bmiCategory === 'Normal' ? '#15803d' : user.bmiCategory === 'Underweight' ? '#b45309' : user.bmiCategory === 'Overweight' ? '#c2410c' : '#c53030',
                    border: user.bmiCategory === 'Normal' ? '1px solid #bbf7d0' : user.bmiCategory === 'Underweight' ? '1px solid #fde68a' : user.bmiCategory === 'Overweight' ? '1px solid #fed7aa' : '1px solid #fed7d7',
                  }}>
                    {user.bmiCategory}
                  </span>
                </div>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  {isEditing ? (
                    <div style={{ display: 'flex', gap: '8px' }}>
                      {BODY_TYPES.map((bt) => (
                        <button key={bt} type="button" onClick={() => updateEdit('bodyType', bt)} style={chipStyle((editForm.bodyType ?? user.bodyType) === bt)}>{bt}</button>
                      ))}
                    </div>
                  ) : (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div>
                  ...
                  {isEditing ? (
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' as const }}>
                      {GOALS.map((g) => (
                        <button key={g} type="button" onClick={() => updateEdit('goal', g)} style={chipStyle((editForm.goal ?? user.goal) === g)}>{g}</button>
                      ))}
                    </div>
                  ) : (
                    <div style={{ ...valueStyle, display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <Target size={14} color="#8a9bc4" /> {user.goal}
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' as const }}>
                      {PREFERENCES.map((p) => (
                        <button key={p} type="button" onClick={() => updateEdit('preference', p)} style={chipStyle((editForm.preference ?? user.preference) === p)}>{p}</button>
                      ))}
                    </div>
                  ) : (
                  ...
                  {isEditing ? (
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' as const }}>
                      {ALLERGY_OPTIONS.map((a) => (
                        <button key={a} type="button" onClick={() => toggleEditAllergy(a)}
                          style={{
                            ...chipStyle((editForm.allergies ?? user.allergies).includes(a)),
                            ...((editForm.allergies ?? user.allergies).includes(a) ? { background: '#fff5f5', border: '2px solid #fc8181', color: '#c53030' } : {}),
                          }}
                        >{a}</button>
                      ))}
                    </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  ...
                  {isEditing ? (
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' as const }}>
                      {CONDITIONS.map((c) => (
                        <button key={c} type="button" onClick={() => toggleEditCondition(c)} style={chipStyle((editForm.conditions ?? user.conditions).includes(c))}>{c}</button>
                      ))}
                    </div>
                  ) : (
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' as const }}>
                ...
                <div>
                  ...
                  {isEditing ? (
                    <input style={inputStyle} value={editForm.medications ?? ''}
                      onChange={(e) => updateEdit('medications', e.target.value)}
                      onFocus={(e) => { e.target.style.border = '2px solid #1a73e8'; e.target.style.background = '#fafcff' }}
                      onBlur={(e) => { e.target.style.border = '2px solid #e8eef8'; e.target.style.background = '#f8fafd' }}
                    />
                  ) : (
                    <div style={{ ...valueStyle, display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <Pill size={14} color="#8a9bc4" /> {user.medications || '—'}
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <div style={{ ...valueStyle, display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
                      <FileText size={14} color="#8a9bc4" style={{ marginTop: '2px', flexShrink: 0 }} />
                      {user.notes || '—'}
                    </div>
                  )}
                </div>
        ...
        {activeTab === 'dietplan' && (
          <div style={cardStyle}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '24px' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: '#fefce8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <UtensilsCrossed size={17} color="#ca8a04" />
              </div>
            ...
            {user.status === 'no-plan' || user.status === 'inactive' ? (
              <div style={{ textAlign: 'center', padding: '60px 40px' }}>
                <div style={{ width: '72px', height: '72px', borderRadius: '22px', background: '#f8fafd', border: '1px solid #e8eef8', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                  <UtensilsCrossed size={30} color="#d0d8f0" />
                </div>
                ...
                <div style={{ fontSize: '13px', color: '#b0bdd8', fontWeight: '500', marginBottom: '24px' }}>
                  Assign a diet plan to help this patient reach their goal
                </div>
                <button
                  onClick={() => navigate(`/users/${id}/plan`)}
              ...
              <div style={{ padding: '20px', borderRadius: '16px', background: '#f0fdf4', border: '1px solid #bbf7d0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button
                  onClick={() => navigate(`/users/${id}/plan`)}
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 18px', borderRadius: '12px', background: 'white', border: '1px solid #bbf7d0', color: '#15803d', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}
                >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ padding: '20px', borderRadius: '16px', background: '#f8fafd', border: '1px solid #e8eef8', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 16px', borderRadius: '12px', background: copied === 'userId' ? '#f0fdf4' : '#eef3ff', border: copied === 'userId' ? '1px solid #bbf7d0' : '1px solid #dbe8ff', color: copied === 'userId' ? '#15803d' : '#1a73e8', fontSize: '13px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s' }}
                >
                  {copied === 'userId' ? <Check size={14} /> : <Copy size={14} />}
              ...
              <div style={{ padding: '20px', borderRadius: '16px', background: '#f8fafd', border: '1px solid #e8eef8', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button
                  onClick={() => handleCopy(user.userEmail, 'email')}
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 16px', borderRadius: '12px', background: copied === 'email' ? '#f0fdf4' : '#eef3ff', border: copied === 'email' ? '1px solid #bbf7d0' : '1px solid #dbe8ff', color: copied === 'email' ? '#15803d' : '#1a73e8', fontSize: '13px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s' }}
                >
              ...
              <div style={{ padding: '20px', borderRadius: '16px', background: '#f8fafd', border: '1px solid #e8eef8', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: '800', color: '#0d1b3e', letterSpacing: '3px', fontFamily: 'monospace' }}>
                    {showPassword ? (user.password ?? '(not stored)') : '••••••••'}
                  </div>
                </div>
                <button
                  onClick={() => setShowPassword(!showPassword)}
                  style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 16px', borderRadius: '12px', background: '#f8fafd', border: '1px solid #e8eef8', color: '#8a9bc4', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}
                >
              ...
              <div style={{ padding: '16px 20px', borderRadius: '14px', background: '#f8fafd', border: '1px solid #e8eef8', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '13px', fontWeight: '700', color: '#0d1b3e' }}>
